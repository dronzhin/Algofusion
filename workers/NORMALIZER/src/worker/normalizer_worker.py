...
from src.modules.normalizer import NormalizerModule
...
class NormalizerWorker:
    ...
    def process_job(self, payload: str) -> bool:
        ...
        if success:
            ...
            job_logger.info("Normalizer completed successfully")
        else:
            job_logger.error("Normalizer failed")
        return success

    def run(self) -> None:
        if not self.connect():
            sys.exit(1)
        logger.info("Normalizer worker started, queue: %s", self.queue_name)
        ...
        logger.info("Normalizer worker stopped")


def main() -> None:
    config.validate()
    NormalizerWorker().run()
