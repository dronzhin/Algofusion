...
def reconcile_document_amounts(doc_type, payload, tol=0.02):
    ...
    payload: внутренний dict документа, где есть items / totals

    Ничего не перезаписывает, если значение уже извлечено.
    Восстанавливает только пустые numeric-поля.
    Может исправлять уже извлечённое число только если:
    - есть минимум 2 независимые совместимые опоры
    - опоры согласованы между собой
    - текущее значение похоже на типичную OCR-ошибку масштаба (46.50 -> 4650)
    """
