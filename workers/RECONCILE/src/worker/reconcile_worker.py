...
from src.modules.reconcile import ReconcileModule
...
class ReconcileWorker:
    ...
    def process_job(self, payload: str) -> bool:
        ...
        if success:
            ...
            job_logger.info("Reconcile completed successfully")
        else:
            job_logger.error("Reconcile failed")
        return success

    def run(self) -> None:
        if not self.connect():
            sys.exit(1)
        logger.info("Reconcile worker started, queue: %s", self.queue_name)
