...
from src.modules.cleaner import CleanerModule
...
class CleanerWorker:
    ...
    def run(self) -> None:
        ...
        logger.info("Cleaner worker started, queue: %s", self.queue_name)
        error_count = 0
        ...
        while not self.shutdown_requested:
            try:
                ...
                _, payload = item
                success = self.process_job(payload.decode("utf-8") if isinstance(payload, bytes) else payload)
